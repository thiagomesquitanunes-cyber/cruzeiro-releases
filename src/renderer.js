...
function aiLink(url) {
  return `<a href="#" onclick="ff.openExternal('${url}');return false;" style="color:var(--accent);text-decoration:underline">${url}</a>`;
}
const AI_PROVIDER_HELP = {
  openrouter: () => `Grátis e sem cartão de crédito. Acesse ${aiLink('https://openrouter.ai/keys')}, crie uma conta e gere uma chave (começa com "sk-or-..."). Usa um roteador de modelos gratuitos por baixo dos panos, de forma estável.`,
  gemini:    () => `⚠️ O Google está emitindo chaves no formato "AQ." para algumas contas, que não funcionam com este tipo de integração (problema atual do Google, não do Cruzeiro). Se sua chave começar com "AQ.", use o OpenRouter acima em vez disso. Caso sua chave comece com "AIza...", ela deve funcionar normalmente. Gere em ${aiLink('https://aistudio.google.com/apikey')}.`,
  openai:    () => `Requer conta com API paga (créditos). Acesse ${aiLink('https://platform.openai.com/api-keys')} e gere uma chave (começa com "sk-...").`,
  anthropic: () => `Requer conta com API paga (créditos). Acesse ${aiLink('https://console.anthropic.com')} e gere uma chave (começa com "sk-ant-...").`,
};
function aiProviderChanged() {
  const prov = G('settings-ai-provider')?.value || 'openrouter';
  const help = G('settings-ai-help');
  const helpFn = AI_PROVIDER_HELP[prov];
  if (help) help.innerHTML = helpFn ? helpFn() : '';
}
async function refreshAiKeyStatus() {
  try {
    ...
    const provSel  = G('settings-ai-provider');
    if (provSel && st.provider) provSel.value = st.provider;
    aiProviderChanged();
    if (st.hasKey) {
      const provName = { openrouter:'OpenRouter', gemini:'Google Gemini', openai:'OpenAI', anthropic:'Anthropic' }[st.provider] || st.provider;
      if (statusEl) { statusEl.textContent = '✅ ' + provName + ' (' + st.masked + ')'; statusEl.style.color = 'var(--green)'; }
      ...
      if (input) input.placeholder = 'cole sua chave aqui';
    }
  } catch(e) {}
}
async function saveAiKey() {
  const input = G('settings-ai-key');
  const provider = G('settings-ai-provider')?.value || 'gemini';
  ...
  // Validação leve de formato por provedor (apenas aviso, não bloqueia)
  const expected = { openrouter:'sk-or-', gemini:'AIza', openai:'sk-', anthropic:'sk-ant-' }[provider];
  if (expected && !key.startsWith(expected)) {
    if (msg) { msg.textContent = '⚠️ A chave deste provedor normalmente começa com "' + expected + '". Salvando mesmo assim.'; msg.style.color = 'var(--warn)'; }
  } else if (msg) { msg.textContent = '✅ Chave salva.'; msg.style.color = 'var(--green)'; }
  await ff.aiSetKey({ provider, key });
...
async function openAiEntry() {
  ...
  if (!st.hasKey) {
    if (!await showConfirmDialog('Configure sua chave de IA', 'A entrada por linguagem natural usa um provedor de IA com sua própria chave (o Google Gemini tem chave gratuita). Deseja abrir as configurações para inseri-la agora?', 'Abrir configurações')) return;
    goPage('backup');
...
async function runAiParse() {
  ...
  if (!res.ok) {
    const errMap = {
      ...
      GEMINI_AQ_KEY: 'Sua chave do Gemini está no formato "AQ." — um problema atual do Google que impede o uso direto. Troque para o provedor OpenRouter em Configurações (também grátis e sem cartão).',
      EMPTY: 'Texto vazio.',
  ...
  // Em vez de um preview somente-leitura, abre o modal de lançamento padrão
  // (já editável, com todos os campos) pré-preenchido para cada item identificado,
  // um por vez em fila — o usuário confere/corrige e confirma normalmente.
  _aiQueuedTxs = txs;
  closeModal('modal-ai-entry');
  toast(`✨ ${txs.length} lançamento(s) identificado(s) — confira e confirme`);
  openNextAiQueuedTx();
}
...
function openNextAiQueuedTx() {
  if (_aiQueueIndex >= _aiQueuedTxs.length) {
  ...
  const t = _aiQueuedTxs[_aiQueueIndex];
  const accId = accounts.find(a => a.id === t.account_id) ? t.account_id : (currentAccountId || accounts[0]?.id);
  // Pré-preenche o modal padrão de lançamento — totalmente editável, sem travas de conta.
  openTxModal({
  ...
  // Marca que este save deve avançar a fila, e ajusta o rótulo do modal
  window._aiQueueActive = true;
  const remaining = _aiQueuedTxs.length - _aiQueueIndex;
  const titleEl = G('modal-tx-title');
  if (titleEl) titleEl.textContent = `✨ Lançamento via IA (${_aiQueueIndex + 1}/${_aiQueuedTxs.length})${remaining > 1 ? '' : ''}`;
}
...
async function maybeShowInsightsCard() {
  ...
  const curRealMonth = todayStr().slice(0,7);
  if (_insightsCache.month === curM && _insightsCache.data) {
    renderInsights(_insightsCache.data);
  } else if (curM >= curRealMonth) {
    if (body) body.innerHTML = '<div style="font-size:13px;color:var(--text3)">Os insights ficam disponíveis para meses já encerrados — navegue para um mês anterior para gerá-los.</div>';
  } else if (body) {
...
async function generateInsights(force, usePersonalization) {
  ...
  const targetMonth = overviewMonthStr(); // respeita o mês aberto na Visão Geral
  const curRealMonth = todayStr().slice(0,7);

  // Nunca analisa o mês corrente — ele ainda está incompleto e distorceria os números
  if (targetMonth >= curRealMonth) {
    body.innerHTML = `<div style="font-size:13px;color:var(--warn)">⚠️ Os insights só funcionam para meses já encerrados (o mês atual ainda não tem todos os dados). Navegue para um mês anterior na Visão Geral e clique em Gerar novamente.</div>`;
    return;
  }
  ...
  try {
    ...
    if (!res.ok) {
      const errMap = { NO_KEY:'Configure a chave de IA em Configurações.',
        ...
        GEMINI_AQ_KEY: 'Sua chave do Gemini está no formato "AQ." (problema atual do Google). Troque para o provedor OpenRouter em Configurações.',
        PARSE_FAIL:'A IA respondeu em formato inesperado. Tente novamente em alguns segundos.',
        API_ERROR:'Erro na API: ' + (res.detail || ''), NETWORK:'Falha de conexão.' };
...
async function saveTx() {
  ...
  window._aiQueueJustSaved = true; // diferencia "salvou" de "cancelou" no closeModal
  closeModal('modal-tx');
  await loadAccounts();
  refreshUndoBtn();
  ...
  if (window._aiQueueActive) {
    _aiQueueIndex++;
    setTimeout(openNextAiQueuedTx, 250); // pequena pausa para o modal fechar visualmente
  }
}
...
async function saveTransfer() {
  ...
  // Se veio de uma linha de importação ainda não confirmada, marca a linha como
  // "já processada" (transferência já criada de fato) — remove visualmente da tabela
  ...
  if (importIdxField?.value !== '') {
    ...
    // Remove a linha da tabela visível (já não há nada a decidir nela)
    const rowEl = document.querySelector(`tr[data-row-idx="${idx}"]`)
      || G(`import-cat-${idx}`)?.closest('tr');
    if (rowEl) {
      rowEl.style.transition = 'opacity .3s, background .3s';
      rowEl.style.background = '#f0fdf4';
      rowEl.style.opacity = '0.5';
      const catCell = rowEl.querySelector('.import-cat-inp');
      if (catCell) { catCell.disabled = true; catCell.value = '✅ Transferência criada'; }
      const memoCell = rowEl.querySelector('.import-memo-inp');
      if (memoCell) memoCell.disabled = true;
    }
    persistImportState();
    importIdxField.value = '';
  }
...
const BUILTIN_CARDS = [
  ...
  { id:'itau_card',type:'card', name:'Itaú (cartão CSV)',fileType:'CSV', sort_order:2,
    logoUrl:'data:image/png;base64,', logoBg:'#EC7000', logoFallback:'IT' },
  { id:'santander_fatura',type:'card', name:'Santander (fatura PDF)',fileType:'PDF', sort_order:3,
    logoUrl:'data:image/png;base64,', logoBg:'#fff', logoFallback:'SF' },
  { id:'itau_fatura',type:'card', name:'Itaú (fatura PDF)',fileType:'PDF', sort_order:4,
    logoUrl:'data:image/png;base64,', logoBg:'#EC7000', logoFallback:'IT' },
];
...
function parseBTGBroker(buffer) {
  ...
  // XP broker statement classification
  // Returns: { flow: 'external'|'income'|'ignore'|'tax', sign: +1|-1 }
  // 'tax' is a special sub-case of external: IRRF/IOF reduce the resgate amount
  // (subtracted from the same month's outflow total) rather than appearing as a
  // separate cost — this yields a net-of-tax redemption value for TWR purposes.
  function classifyXPFlow(desc) {
    const d = norm(String(desc || ''));

    // Ignore: bank transfers (handled by bank import separately) and generic entries
    if (d.includes('ted bco') && d.includes('retirada em c/c')) return { flow: 'ignore' };
    if (d.trim() === 'lancamento') return { flow: 'ignore' };
    if (d.includes('come cota') || d.includes('come-cota')) return { flow: 'ignore' };

    // Taxes on redemption — reduce the resgate (external outflow), net of tax
    if (d.includes('irrf') && d.includes('resgate'))  return { flow: 'tax', sign: -1 };
    if (d.includes('iof')  && d.includes('resgate'))  return { flow: 'tax', sign: -1 };

    // Income flows — distributions/returns generated by the asset
    if (d.includes('rendimento fundo fechado balcao')) return { flow: 'income', sign: +1 };
    if (d.includes('devolucao tx de distr'))           return { flow: 'income', sign: +1 };

    // External capital flows
    // Amortização de fundo = return of principal — treated as venda (external outflow)
    if (d.includes('amortizacao de fundo'))            return { flow: 'external', sign: +1 };
    // Integralização de cotas = compra (external inflow, money leaving investor's pocket)
    if (d.includes('integralizacao de cotas'))         return { flow: 'external', sign: -1 };
    // Adiantamento resgate / resgate = venda (external outflow, money returning to investor)
    if (d.includes('adiantamento resgate'))            return { flow: 'external', sign: +1 };
    if (d.includes('resgate'))                         return { flow: 'external', sign: +1 };

    return { flow: null, sign: null }; // unknown — caller decides fallback
  }
...
function parseXPBroker(posicaoBuffer, extratoBuffer) {
  ...
  for (let ri = dataStart; ri < extRows.length; ri++) {
    ...
    if (cls.flow === 'tax') {
      ...
      // |rawVal| is the tax amount; subtract from the corresponding venda's outflow
      const taxAmt = Math.abs(rawVal);
      target.movimentacoes.push({
        amount: -taxAmt, type: desc, flow_type: 'external',
      });
      continue;
    }
    ...
    if (cls.flow === 'income') {
      ...
      // Detect total liquidation: a venda/resgate close to the asset's full position value
      if (cls.sign > 0 && desc.toUpperCase().includes('RESGATE') && !desc.toUpperCase().includes('ADIANTAMENTO')) {
...
async function onWizardFileSelected(event) {
  ...
  } catch(e) { console.error('Import auto-detect error:', e); /* fall through to manual */ }
...
// Uses pdf.js positional text extraction (x,y coordinates) since plain text
// extraction interleaves columns incorrectly.
...
function pickGlobalCat(val) {
  ...
  if (tfMatch) {
    ...
    // Se a categorização ocorreu na tela de edição de importação (extrato ainda não salvo),
    // preenche o modal a partir da própria linha importada (data, valor, memo já conhecidos).
    const importIdMatch = _globalCatActiveInput?.id?.match(/^import-cat-(\d+)$/);
    if (importIdMatch) {
      const idx = parseInt(importIdMatch[1]);
      const row = _importEditRows[idx];
      if (row) { row._importIdx = idx; openTransferFromImportRow(row, destAcc); return; }
    }
    // Caminho padrão: categorização inline de uma transação já salva no extrato
...
// Preenche o modal de transferência automaticamente a partir de uma linha
// ainda em importação (não salva): usa data, valor e conta de origem/destino
// conforme o sinal do lançamento, e o memorando sugerido pelo ML quando houver.
function openTransferFromImportRow(row, destAcc) {
  setupCurrencyInput(G('tr-amount'));

  const srcAccId = row.accountId ?? currentAccountId;
  const isDebit  = row.amount < 0; // saída desta conta = ela é a origem; entrada = ela é o destino

  const fromSel = G('tr-from');
  const toSel   = G('tr-to');
  if (isDebit) {
    if (fromSel && srcAccId) fromSel.value = String(srcAccId);
    if (toSel && destAcc)    toSel.value   = String(destAcc.id);
  } else {
    if (fromSel && destAcc)   fromSel.value = String(destAcc.id);
    if (toSel && srcAccId)    toSel.value   = String(srcAccId);
  }
  ...
  // se o usuário também confirmar a importação inteira depois.
  const hidField = G('tr-origin-tx-id');
  if (hidField) hidField.value = '';
  let importIdxField = G('tr-import-row-idx');
  if (!importIdxField) {
    importIdxField = document.createElement('input');
    importIdxField.type = 'hidden'; importIdxField.id = 'tr-import-row-idx';
    G('modal-transfer')?.appendChild(importIdxField);
  }
  importIdxField.value = row._importIdx ?? '';
...
// TIR real por ativo individual — mesma lógica usada na tabela de Patrimônio (buildInvRows),
// extraída para reuso (ex.: sugestões de liquidação na Projeção de Fluxo de Caixa).
// txs: transações do ativo (qualquer formato com tx_type, month, total_value)
// ipcaCumFn: função(mês) → fator de correção IPCA acumulado até curM
// curM: mês corrente 'YYYY-MM'
function calcAssetRealIRR(txs, ipcaCumFn, curM) {
  if (!txs.length) return null;
  const txMonths = [...new Set(txs.map(t => t.month.slice(0,7)))].sort();
  if (!txMonths.length) return null;
  ...
  txMonths.forEach(m => {
    ...
    txs.filter(t => t.month.slice(0,7) === m).forEach(t => {
      if (t.tx_type in INV_TX_EXTERNAL) {
        const sign = INV_TX_EXTERNAL[t.tx_type].sign;
        monthlyNetCash[m] = (monthlyNetCash[m] || 0) + sign * t.total_value;
      } else if (t.tx_type in INV_TX_INCOME) {
    ...
    if (lastValuation !== null) { running = lastValuation; bookValueByM[m] = running; }
  });

  // Propaga o valor contábil até o mês corrente
  let lastVal = 0;
  const allM = [];
  let m = txMonths[0];
  while (m <= curM) { allM.push(m); const [y,mo]=m.split('-').map(Number); m = mo===12?`${y+1}-01`:`${y}-${String(mo+1).padStart(2,'0')}`; }
  allM.forEach(mm => { if (bookValueByM[mm] !== undefined) lastVal = bookValueByM[mm]; });
  const latestVal = bookValueByM[curM] ?? lastVal;

  const hasCashFlows = Object.keys(monthlyNetCash).length > 0;
  if (!hasCashFlows) return null;

  const realFlows = allM.map(mm => (monthlyNetCash[mm] ?? 0) * ipcaCumFn(mm));
  realFlows[realFlows.length - 1] += latestVal; // venda hipotética hoje
...
function aposResetConfig() {
  ...
  const r = document.querySelector('input[name="apos-goal-type"][value="renda"]');
  if (r) r.checked = true;
