...
const BUILTIN_CARDS = [
  ...
  { id:'santander_fatura',type:'card', name:'Santander (fatura PDF)',fileType:'PDF', sort_order:3,
    logoUrl:'', logoBg:'#EC0000', logoFallback:'SF' },
];
...
function pickImport(bankId, type) {
  ...
  G('bank-file-input').accept = bankId === 'xp' ? '.csv,.CSV' : bankId === 'santander_fatura' ? '.pdf,.PDF' : '.xls,.xlsx,.XLS,.XLSX';
  G('bank-file-trigger').style.display = 'flex';
...
const TOUR_STEPS = [
  ...
  // ── 19. Bens imobilizados ─────────────────────────────────────────────────
  {
    ...
    body: `Clique em <strong>+ Ativo</strong> para cadastrar um bem físico.<br><br>
      • Escolha o índice de correção: <strong>IPCA, IGP-M, INCC ou TR</strong><br>
      • Vincule lançamentos bancários ao bem (aluguéis, parcelas, dividendos…) — eles são contabilizados na TIR do ativo<br>
      • Para bens financiados: gere o <strong>cronograma de amortização</strong> (SAC, PRICE, SAM ou Planta) — o saldo devedor diminui automaticamente quando você registra parcelas pagas<br>
  ...
  // ── 27. Exportar dados ───────────────────────────────────────────────────
  {
    ...
    body: `Em Configurações → <strong>Exportar dados</strong>:<br><br>
      ...
      • <strong>CSV por conta</strong> — exporta as transações de cada conta em planilha. Compatível com Excel, Google Sheets e qualquer outro app financeiro.<br><br>
  ...
  // ── 28. Categorias ────────────────────────────────────────────────────────
  {
    ...
    body: `Em Configurações → <strong>Categorias</strong>, gerencie sua estrutura de categorias.<br><br>
  ...
  // ── 29. Licença ───────────────────────────────────────────────────────────
  {
    ...
    body: `O Cruzeiro é <strong>gratuito por 6 meses</strong> para qualquer pessoa.<br><br>
      Após esse período, permanece <strong>100% gratuito</strong> se <em>qualquer</em> critério for atendido:<br>
